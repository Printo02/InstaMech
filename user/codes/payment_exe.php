...
if($exe!=null)
{
        ...
        if($camount>=$amount)
        {
          ...
          $qrya="Select * from bank where user_type='admin' ";
          $exea=$obj->GetSingleRow($qrya);  

          $adminamount=$exea['amount'];

         $ammm= $adminamount+$amount;

          $sql3="update bank set amount='$ammm' where user_type='admin'";
     ...
     $sqlqry = "UPDATE servicerequest SET status='payed', amount='$amount' WHERE s_id='$s_id'";
        ...
        // die;
         ...
         echo $obj->alert("Payment Successfull","../payed_service.php");
             }   
                else
                {
                    
    echo $obj->alert("Insufficient Balance ","../my_service_request.php");
                }
